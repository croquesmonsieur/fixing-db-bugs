...
if(!empty($_POST['firstname']) && !empty($_POST['lastname'])) {
    ...
    if(empty($_POST['id'])) {
        ...
        $handle = $pdo->prepare('UPDATE user u SET firstname = :firstname, lastname = :lastname, year = :year WHERE id = :id');
    ...
    if(!empty($_POST['id'])) {
        ...
        $userId = $pdo->lastInsertId(); //why did I leave this if empty? There must be no important reason for this. Move on.
    }
...
elseif(isset($_POST['delete'])) {
    ...
    $handle = $pdo->prepare('DELETE FROM user u where u.id = :id ');
...
//@todo Invalid query?
$handle = $pdo->prepare('SELECT u.id, concat_ws(" ", firstname, lastname) AS name, group_concat(" ", s.sport) as sport FROM user u left JOIN sport s ON u.id = s.user_id where year = :year group by u.id');
$handle->bindValue(':year', date('Y'));
$handle->execute();
...
if(!empty($_GET['id'])) {
    ...
    foreach($handle->fetchAll() AS $sport) {
        $selectedUser['sports'][] = implode('', $sport);//@todo I just want an array of all sports of this, why is it not working?
    }
}
